...
const query = {
  ...
  ATTENDANCE_DETAIL: "SELECT  att.id, att.roll_number, att.register_number, att.name, att.department, ld.lab_name, ld.lab_department, att.section, att.semester, att.batch, att.academic_year  ,DATE_FORMAT(att.logintime,'%d-%m-%Y') date,DATE_FORMAT(att.logintime,'%h:%i:%s') logintime,  DATE_FORMAT(att.logouttime,'%h:%i:%s') logouttime, att.machine_no , att.lab_id FROM attendances as att INNER JOIN lab_details as ld WHERE att.lab_id = ld.lab_id AND  ld.lab_name LIKE ? and att.logintime LIKE ? AND  att.batch LIKE ? AND academic_year LIKE ? AND semester LIKE ? AND att.department LIKE ? AND section LIKE ? AND att.name LIKE ?;",
  ...
  UPDATE_STUDENT_PASSWORD: "UPDATE students SET password = ? WHERE students.roll_number = ? ;",
